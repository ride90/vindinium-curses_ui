...
class AI(AIBase):
    ...
    def decide(self):
        ...
        decisions = {
            "mine": [("Mine", 30), ("Fight", 10), ("Tavern", 5)],
        ...
        for y in range(self.game.board_size):
            for x in range(self.game.board_size):
                if (
                    (y, x) not in self.game.walls_locs
                    or (y, x) not in self.game.taverns_locs
                    or (y, x) not in self.game.mines_locs
                ):
                    walkable.append((y, x))
        ...
        for i in range(int(round(random.random() * self.game.board_size))):
            for i in range(len(walkable)):
                ...
                if (
                    walkable[i][0] - first_cell[0] == 1
                    and walkable[i][1] - first_cell[1] == 0
                ) or (
                    walkable[i][1] - first_cell[1] == 1
                    and walkable[i][0] - first_cell[0] == 0
                ):
